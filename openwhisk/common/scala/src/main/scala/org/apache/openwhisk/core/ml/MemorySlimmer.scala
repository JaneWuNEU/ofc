package org.apache.openwhisk.core.ml
...
import weka.core.{DenseInstance, FastVector, Instance, Instances, SerializationHelper}
...
trait MemorySlimmer {
  ...
  def slim(action: WhiskActionMetaData, payload: Option[JsObject], memUsed: Option[Int])(logging: Logging): MemoryLimit
}

/** Dummy memory slimmer that does not change the memory limit. */
object NoChangeMemorySlimmer extends MemorySlimmer {
  override def slim(action: WhiskActionMetaData, payload: Option[JsObject], memUsed: Option[Int])(logging: Logging): MemoryLimit = {
...
object FinalMemSlimmer extends MemorySlimmer {
  ...
  def languageSupported(action: WhiskActionMetaData)(logging: Logging) : Option[Boolean] = {
    logging.info(this,s"[FAASCache] checking language support for current action")
    action.exec.kind.split(':').head match {
      case "python" => Some(true) 
      case "nodejs" => Some(false)
      case "blackbox" => 
  ...
  def canPredict(action: WhiskActionMetaData)(logging: Logging): Option[Boolean] = {
      logging.info(this,s"Checking if a model already exists for current action")
  ...
  override def slim(action: WhiskActionMetaData, payload: Option[JsObject], memUsed: Option[Int])(logging: Logging): Option[MemoryLimit] = {
    ...
    inst.set(0, arguments("input_size"))
    val counter = 0
    functionArgumentsProcessors(FUNCTION_NAMES_TO_IDS(action.name.asString)).foreach {
        ...
        case (argname, argprocfact) =>
          val argproc = argprocfact.build(arguments(argname))
    ...
    Files.exists(Paths.get(s"$getFunctionId.model")) match {
      case false => {
          ...
          inst.set(counter+1, memUsed.getOrElse(0))
          logging.info(this, s"Feeding $getFunctionId.arff with new entry ")

           Files.exists(Paths.get(s"$getFunctionId.arff")) match {
            case false => {
               val arffSaver = new ArffSaver
               val newInstance = new Instances 
               val atts = new FastVector 
               
               //Get the arguments passed to the function 
               arguments.keySet.foreach((item: String)=> atts.addElement(new Attribute(item)))
               atts.addElement(new Attribute("memory"))
//             atts.addElement(new Attribute("input_size"))

               arffSaver.setInstances(inst)
               arffSaver.setFile(s"$getFunctionId.arff")
               arffSaver.writeBatch()
               
            }

            case true => {
              val funcDataset = loadDataStructure(s"$getFunctionId.arff")
