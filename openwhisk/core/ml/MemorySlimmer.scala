...
trait MemorySlimmer {
  ...
  def slim(action: WhiskActionMetaData, payload: Option[JsObject])(logging: Logging): MemoryLimit
}

/** Dummy memory slimmer that does not change the memory limit. */
object NoChangeMemorySlimmer extends MemorySlimmer {
  override def slim(action: WhiskActionMetaData, payload: Option[JsObject])(logging: Logging): MemoryLimit = {
...
object finalMemSlimmer extends MemorySlimmer {
  ...
  override def slim(action: WhiskActionMetaData, payload: Option[JsObject])(logging: Logging): Memory = {
