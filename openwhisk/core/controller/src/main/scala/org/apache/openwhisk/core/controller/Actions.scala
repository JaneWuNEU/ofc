...
import org.apache.openwhisk.core.FinalMemSlimmer
...
trait WhiskActionsApi extends WhiskCollectionAPI with PostActionActivation with ReferencedEntities {
  ...
  /** Memory Slimmer instance for memory predictions and model feeding/training **/ 

  protected val memSlimmer: FinalMemSlimmer
            ...
            act: WhiskActionMetaData =>
              ...
              onComplete(entitleReferencedEntitiesMetaData(user, Privilege.ACTIVATE, Some(action.exec))) {
                case Success(_) =>
                  ...
                  if (allowInvoke) {
                    //Here we must predict the memory to be used by the action 
                    //Two cases, a model already exists => we run the prediction to get the memory or 
                    // no model exists => we feed the dataset to later on train and generate a corresponding model 
                    if(memSlimmer.canPredict(actionWithMergedParams)){
                      
                      val predictedMem = memSlimmer.slim(actionWithMergedParams,payload,None).getOrElse(MemoryLimit(0))
                      val toPlugInMemory = actionWithMergedParams.limits.memory.getOrElse(MemoryLimit(0)).megabytes - predictedMem.megabytes

                      //Routine to plug the exceeding memory in the cache 
                      //rcScalaClient.scaleUp(toPlugInMemory)

                      actionWithMergedParams.limits.memory = predictedMem
                      doInvoke(user,actionWithMergedParams,payload,blocking,waitOverride,result)
                      
                    }else{
                      doInvoke(user, actionWithMergedParams, payload, blocking, waitOverride, result,true)
                    }
  ...
  private def doInvoke(user: Identity,
                       ...
                       result: Boolean,
                       feed: Boolean = false)(implicit transid: TransactionId): RequestContext => Future[RouteResult] = {
