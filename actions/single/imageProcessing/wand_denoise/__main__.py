...
def main(args):
    ...
    image, threshold, softness = args.get('img','1KB.jpg'), 50, 50
    ...
    with Image(blob=imgstream) as img:
        img.wavelet_denoise(
                threshold=threshold * img.quantum_range, softness=softness)
        ...
        with open('out.jpg', 'rb') as local:
            conn.put_object(container_name, 'resDenoise'+str(random.randrange(0,100,2))+'.jpg', contents=local, content_type="image/jpeg")
